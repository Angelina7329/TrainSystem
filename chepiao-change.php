		...
		<div class="main-content">
			...
			<div class="panel panel-default">
				...
				<div class="panel-heading">
					<div>
						<font style="font-family:'宋体';font-size:150%;color:#2C2E2F">根据条件搜索车票信息</font>
					</div>
				</div>
				
				<div class = "panel-body">
				
					<form role="form" action="chepiao-change.php" method="post">
							<div class = "row">
								
								<div class = "col-md-4">
								<p><span style = "color:#2C2E2F">车票编号：</span><input typen = "text" name = "ticketnumcer" placeholder="如：nf877" style = "color:#2C2E2F"
								value = "<?php if (!empty($_POST['ticketnumcer']))echo $_POST['ticketnumcer'];?>"/></p>
								</div>
								<div class = "col-md-4">
								<p><span style = "color:#2C2E2F">始发站点：</span><input typen = "text" name = "origin" placeholder="如：济南" style = "color:#2C2E2F"
								value = "<?php if (!empty($_POST['origin']))echo $_POST['origin'];?>"/></p>
								</div>
								<div class = "col-md-4">
								<p><span style = "color:#2C2E2F">目的站点：</span><input typen = "text" name = "destination" placeholder="如：青岛" style = "color:#2C2E2F"
							...
							<div class = "row">
							<div class = "col-md-4">
								<p><span style = "color:#2C2E2F">车厢编号：</span><input typen = "text" name = "carriage" placeholder="如：8" style = "color:#2C2E2F"
								value = "<?php if (!empty($_POST['carriage']))echo $_POST['carriage'];?>"/></p>
								</div>
								<div class = "col-md-4">
								<p><span style = "color:#2C2E2F">座位编号：</span><input typen = "text" name = "seat" placeholder="如：24" style = "color:#2C2E2F"
								value = "<?php if (!empty($_POST['seat']))echo $_POST['seat'];?>"/></p>
								</div>
								<div class = "col-md-4">
								<p><span style = "color:#2C2E2F">座位类型：</span><input typen = "text" name = "seattype" placeholder="如：一等座" style = "color:#2C2E2F"
								value = "<?php if (!empty($_POST['seattype']))echo $_POST['seattype'];?>"/></p>
								</div>
							</div>
							<div class="form-group-separator"></div>
							<div class = "row">
							<div class = "col-md-4">
								<p><span style = "color:#2C2E2F">车票价格：</span><input typen = "text" name = "price" placeholder="如：100" style = "color:#2C2E2F"
								value = "<?php if (!empty($_POST['price']))echo $_POST['price'];?>"/></p>
								</div>
							<div class = "col-md-5">
							</div>
							<div class = "col-md-3">
				...
				<div class="row">
								...
								<table class="table table-bordered table-striped" id="SortTable">
										...
										<?php
											...
											$add = 0;
											if(!empty($_POST['ticketnumcer'])){
												if($add == 0){
													$q = $q.' WHERE TicketNumber = "'.$_POST['ticketnumcer'].'" ';
													$add++;
												}else{
													$q = $q.'and TicketNumber = "'.$_POST['ticketnumcer'].'" ';
												}
											}
											...
											if(!empty($_POST['carriage'])){
												if($add == 0){
													$q = $q.' WHERE Carriage = "'.$_POST['carriage'].'" ';
													$add++;
												}else{
													$q = $q.'and Carriage = "'.$_POST['carriage'].'" ';
												}
											}
											
											if(!empty($_POST['seat'])){
												if($add == 0){
													$q = $q.' WHERE Seat = "'.$_POST['seat'].'" ';
													$add++;
												}else{
													$q = $q.'and Seat = "'.$_POST['seat'].'" ';
												}
											}
											
											if(!empty($_POST['seattype'])){
												if($add == 0){
													$q = $q.' WHERE SeatType = "'.$_POST['seattype'].'" ';
													$add++;
												}else{
													$q = $q.'and SeatType = "'.$_POST['seattype'].'" ';
												}
											}
											
											if(!empty($_POST['price'])){
												if($add == 0){
													$q = $q.' WHERE price = "'.$_POST['price'].'" ';
													$add++;
												}else{
													$q = $q.'and price = "'.$_POST['price'].'" ';
												}
											}
											...
											if($num>0){
												...
												while($row = mysqli_fetch_array($r,MYSQLI_ASSOC)){
													...
													echo '<tr>
														<td align = "left"><a href = "chepiao-change-edit.php?tn='.$row['TicketNumber'].'">Edit</a></td>
														<td align = "left"><a href = "chepiao-change-delete.php?tn='.$row['TicketNumber'].'">Delete</a></td>
														...
														<td align = "left">'.$row['Carriage'].'</td>
														<td align = "left">'.$row['Seat'].'</td>
														<td align = "left">'.$row['SeatType'].'</td>
