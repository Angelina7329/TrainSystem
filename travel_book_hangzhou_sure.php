...
<section id="recent-works">
  ...
  require("mysql_connect.php");		
		if($_SERVER['REQUEST_METHOD']=='POST'){
			$stl = $_POST['stl'];
			$std = $_POST['std'];
				$q = "SELECT * FROM ticket WHERE StartLocation = '$stl' AND ArrivalLocation = '杭州' AND DepartureTime BETWEEN '$std' AND DATE_ADD('$std',INTERVAL 1 DAY) LIMIT 1";
				$cover = 0;
				$r = @mysqli_query($dbc,$q);
				if(mysqli_affected_rows($dbc)==1){
					$q = "SELECT * FROM ticket WHERE StartLocation = '杭州' AND ArrivalLocation = '$stl' AND DepartureTime BETWEEN DATE_ADD('$std',INTERVAL 4 DAY) AND DATE_ADD('$std',INTERVAL 5 DAY) LIMIT 1";
					//var_dump($q);
					$r = @mysqli_query($dbc,$q);
					if(mysqli_affected_rows($dbc)==1){
						echo "<p>系统中存在你所需要的车票!</p>";
						$cover = 1;
					}else{
				...
				if($cover == 1){
					...
					$q = "SELECT * FROM ticket WHERE StartLocation = '$stl' AND ArrivalLocation = '杭州' AND DepartureTime BETWEEN '$std' AND DATE_ADD('$std',INTERVAL 1 DAY)
							AND TicketNumber NOT IN(
													SELECT TicketNumber 
														FROM ticket NATURAL JOIN ticket_book
															WHERE StartLocation = '$stl' AND ArrivalLocation = '杭州' AND DepartureTime BETWEEN '$std' AND DATE_ADD('$std',INTERVAL 1 DAY)
													)LIMIT 1";
					$r = @mysqli_query($dbc,$q);
					$row = mysqli_fetch_array($r);	
					$TicketNumber = $row[0];
					...
					$q = "SELECT * FROM ticket WHERE StartLocation = '杭州' AND ArrivalLocation = '$stl' AND DepartureTime BETWEEN DATE_ADD('$std',INTERVAL 4 DAY) AND DATE_ADD('$std',INTERVAL 5 DAY) 
					AND TicketNumber NOT IN(
													SELECT TicketNumber 
														FROM ticket NATURAL JOIN ticket_book
															WHERE StartLocation = '杭州' AND ArrivalLocation = '$stl' AND DepartureTime BETWEEN DATE_ADD('$std',INTERVAL 4 DAY) AND DATE_ADD('$std',INTERVAL 5 DAY)
													)LIMIT 1";
					//var_dump($q);
					$r = @mysqli_query($dbc,$q);
					$row = mysqli_fetch_array($r);	
